...
# File upload option
# -----------------------------
uploaded_file = st.file_uploader("Upload CSV/Excel with sample values", type=["csv", "xlsx"])

# -----------------------------
# Manual entry fields (fallback if no file uploaded)
# -----------------------------
...
if st.button("Run Simulation"):
    if uploaded_file is not None:
        ...
        # Expect columns: sensor, temporal, spatial
        sensor_samples = df["sensor"].values
        temporal_samples = df["temporal"].values
        spatial_samples = df["spatial"].values

        total_concentration = sensor_samples + temporal_samples + spatial_samples

    else:
        # Use manual entry + Monte Carlo
